...
const (
	bareIOSProjectPathInputTitle   = "The iOS project path generated ny the 'expo eject' process"
...
const (
	...
	iosBundleIDInputSummary        = "Did not found the key expo/ios/bundleIdentifier in 'app.json'. You can add it now, or commit in the repository later. Needed as only the Expo bare workflow is supported (https://docs.expo.io/bare/customizing/)."
	iosBundleIDInputSummaryDefault = "You can add the iOS bundle identifier now or commit in the repository later. Leave empty if the key expo/ios/bundleIdentifier in 'app.json' is present. Needed as only the Expo bare workflow is supported (https://docs.expo.io/bare/customizing/)."
	iosBundleIDEnvKey              = "EXPO_BARE_IOS_BUNLDE_ID"
)

const (
	androidPackageInputTitle          = "Android package name"
	androidPackageInputSummary        = "Did not found the key expo/android/package in 'app.json'. You can add it now, or commit in the repository later. Needed as only the Expo bare workflow is supported (https://docs.expo.io/bare/customizing/)."
	androidPackageInputSummaryDefault = "You can add the Android package name now or commit in the repository later. Leave empty if the key expo/android/package in 'app.json' is present. Needed as only the Expo bare workflow is supported (https://docs.expo.io/bare/customizing/)."
...
func (Scanner) expoDefaultOptions() models.OptionNode {
	...
	schemeOption.AddOption("", schemeOption)
	...
	userNameOption := models.NewOption(expoUserNameInputTitle, expoUserNameInputSummary, "EXPO_USERNAME", models.TypeUserInput)
	shouldPublishNode.AddOption("Release", userNameOption)

	passwordOption := models.NewOption(expoPasswordInputTitle, expoPasswordInputSummary, "EXPO_PASSWORD", models.TypeUserInput)
	userNameOption.AddOption("", passwordOption)

	shouldPublishNode.AddOption("no", projectPathOptionPublishNo)
	shouldPublishNode.AddOption("yes", projectPathOptionPublishYes)
	...
	for _, s := range []struct{ option, config *models.OptionNode }{
		...
		for _, child := range s.option.LastChilds() {
			for _, lastOption := range child.ChildOptionMap {
				lastOption.AddConfig("", s.config)
			}
		}
	}
