	...
	for _, project := range standaloneProjects {
		...
		if len(project.SharedSchemes) == 0 {
			...
			for _, target := range project.Targets {
				...
				exportMethodOption := models.NewOption(exportMethodInputTitle, exportMethodInputSummary, DistributionMethodEnvKey, models.TypeSelector)
...
func GenerateConfigBuilder(
	...
	xcodeArchiveMacStepInputModels := append(xcodeStepInputModels, envmanModels.EnvironmentItemModel{ExportMethodInputKey: "$" + DistributionMethodEnvKey})
...
func GenerateDefaultConfig(projectType XcodeProjectType, isIncludeCache bool) (models.BitriseConfigMap, error) {
	...
	xcodeArchiveMacStepInputModels := append(xcodeTestStepInputModels, envmanModels.EnvironmentItemModel{ExportMethodInputKey: "$" + DistributionMethodEnvKey})
