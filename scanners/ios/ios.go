...
func filterProjectSharedSchemes(fileList []string, project string) ([]SchemeModel, error) {
...
func (scanner *Scanner) Options() (models.OptionModel, error) {
	...
	podfileWorkspaceMap := map[string]string{}
	for _, podFile := range podFiles {
		...
		for workspace, linkedProject := range podfileWorkspaceMap {
	...
	for _, project := range scanner.XcodeProjectFiles {
		...
		_, found := podfileWorkspaceMap[project]
		if !found {
			cleanProjectFiles = append(cleanProjectFiles, project)
		}
	}
	...
	for _, project := range cleanProjectFiles {
		...
		if isWorkspace {
			...
			workspaceSchemes, err := filterProjectSharedSchemes(scanner.FileList, project)
			if err != nil {
				return models.OptionModel{}, err
			}
			...
			for _, workspaceProject := range workspaceProjects {
				...
				workspaceProjectSchemes, err := filterProjectSharedSchemes(scanner.FileList, workspaceProject)
				if err != nil {
					return models.OptionModel{}, err
				}
			...
			found := utility.MapStringStringHasValue(podfileWorkspaceMap, project)
			if found {
			...
			projectSchemes, err := filterProjectSharedSchemes(scanner.FileList, project)
			if err != nil {
				return models.OptionModel{}, err
			}
