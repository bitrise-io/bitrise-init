...
func (scanner *Scanner) DetectPlatform() (bool, error) {
	...
	log.Details("%d xcodeproj file(s) detected:", len(xcodeProjectFiles))
...
func (scanner *Scanner) Options() (models.OptionModel, models.Warnings, error) {
	...
	for _, project := range cleanProjectFiles {
		...
		if isWorkspace {
			...
			log.Details("workspace shared schemes: %v", workspaceSchemeXCTestMap)
			...
			if len(workspaceSchemeXCTestMap) == 0 {
				...
				log.Details("workspace user schemes: %v", targetXCTestMap)
			...
			if found {
				...
				for workspace, linkedProject := range podfileWorkspaceProjectMap {
					if linkedProject == project {
						log.Details("workspace will be generated by CocoaPods: %s", workspace)
			...
			log.Details("project shared schemes: %v", projectSchemeXCtestMap)
			...
			if len(projectSchemeXCtestMap) == 0 {
				...
				log.Details("project user schemes: %v", targetXCTestMap)
		...
		log.Done("found schemes: %v", schemeXCTestMap)
