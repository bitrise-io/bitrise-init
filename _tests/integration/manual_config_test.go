...
var customConfigVersions = []interface{}{
	...
	// default-react-native-expo-config-publish/deploy
	models.FormatVersion,
	steps.ActivateSSHKeyVersion,
	steps.GitCloneVersion,
	steps.ScriptVersion,
	...
	// default-react-native-expo-config-publish/primary
	steps.ActivateSSHKeyVersion,
	steps.GitCloneVersion,
	steps.ScriptVersion,
	steps.NpmVersion,
    ...
    value_map:
      ...
      "yes":
        ...
        value_map:
          "no":
            ...
            value_map:
              ./ios/< PROJECT NAME >.xcworkspace:
                ...
                summary: 'For example: ''com.sample.myapp''. You can specify the iOS
                  bundle identifier in case the key expo/ios/bundleIdentifier is not
                  set in ''app.json''. Needed to eject to the bare workflow, managed
                  workflow is not supported (https://docs.expo.io/bare/customizing/).'
                ...
                value_map:
                  "":
                    ...
                    summary: An Xcode scheme defines a collection of targets to build,
                      a configuration to use when building, and a collection of tests
                      to execute. You can change the scheme at any time.
                    env_key: BITRISE_SCHEME
                    type: user_input
                    value_map:
                      "":
                        ...
                        value_map:
                          ad-hoc:
                            ...
                            summary: 'For example: ''com.sample.myapp''. You can specify
                              the Android package name in case the key key expo/android/package
                              is not set in ''app.json''. Needed to eject to the bare
                              workflow, managed workflow is not supported (https://docs.expo.io/bare/customizing/).'
                                    ...
                                    value_map:
                                      ./android:
                                        ...
                                        summary: Modules provide a container for your
                                          Android project's source code, resource
                                          files, and app level settings, such as the
                                          module-level build file and Android manifest
                                          file. Each module can be independently built,
                                          tested, and debugged. You can add new modules
                                        ...
                                        value_map:
                                          app:
                                            title: Variant
                                            summary: Your Android build variant. You
                                              can add variants at any time, as well
                                              as further configure your existing variants
                                              later.
                                            env_key: VARIANT
                                            type: user_input_optional
                                            value_map:
          ...
          "yes":
            title: Expo username
            summary: 'Your Expo account username: required to publish using Expo CLI.'
            env_key: EXPO_USERNAME
            type: user_input
            value_map:
              "":
                title: Expo password
                summary: 'Your Expo account password: required to publish using Expo
                  CLI.'
                ...
                value_map:
                  "":
                    title: The iOS project path generated by the 'expo eject' process
                    summary: 'The relative location of the Xcode workspace, after
                      running ''expo eject''. For example: ''./ios/myproject.xcworkspace''.
                      Needed to eject to the bare workflow, managed workflow is not
                    ...
                    value_map:
                      ./ios/< PROJECT NAME >.xcworkspace:
                        title: iOS bundle identifier
                        summary: 'For example: ''com.sample.myapp''. You can specify
                          the iOS bundle identifier in case the key expo/ios/bundleIdentifier
                          is not set in ''app.json''. Needed to eject to the bare
                          workflow, managed workflow is not supported (https://docs.expo.io/bare/customizing/).'
                        env_key: EXPO_BARE_IOS_BUNLDE_ID
                        type: user_input
                        value_map:
                          "":
                            title: The iOS scheme name generated by the 'expo eject'
                              process
                            summary: An Xcode scheme defines a collection of targets
                              to build, a configuration to use when building, and
                              a collection of tests to execute. You can change the
                              scheme at any time.
                            env_key: BITRISE_SCHEME
                            type: user_input
                            value_map:
                              "":
                                ...
                                value_map:
                                  ad-hoc:
                                    title: Android package name
                                    summary: 'For example: ''com.sample.myapp''. You
                                      can specify the Android package name in case
                                      the key key expo/android/package is not set
                                      in ''app.json''. Needed to eject to the bare
                                      workflow, managed workflow is not supported
                                    ...
                                    value_map:
                                      "":
                                        title: Project root directory
                                        summary: The directory of the 'app.json' or
                                          'package.json' file of your React Native
                                          project.
                                        env_key: WORKDIR
                                        type: user_input
                                        value_map:
                                          "":
                                            title: The root directory of an Android
                                              project
                                            summary: The root directory of your Android
                                              project, stored as an Environment Variable.
                                              In your Workflows, you can specify paths
                                              relative to this path. You can change
                                              this at any time.
                                            env_key: PROJECT_LOCATION
                                            type: selector
                                            value_map:
                                              ./android:
                                                title: Module
                                                summary: Modules provide a container
                                                  for your Android project's source
                                                  code, resource files, and app level
                                                  settings, such as the module-level
                                                  build file and Android manifest
                                                  file. Each module can be independently
                                                  built, tested, and debugged. You
                                                  can add new modules to your Bitrise
                                                  builds at any time.
                                                env_key: MODULE
                                                type: user_input
                                                value_map:
                                                  app:
                                                    title: Variant
                                                    summary: Your Android build variant.
                                                      You can add variants at any
                                                      time, as well as further configure
                                                      your existing variants later.
                                                    env_key: VARIANT
                                                    type: user_input_optional
                                                    value_map:
                                                      Release:
                                                        config: default-react-native-expo-config-publish
                                  app-store:
                                    title: Android package name
                                    summary: 'For example: ''com.sample.myapp''. You
                                      can specify the Android package name in case
      ...
      workflows:
        ...
        primary:
          ...
          - npm@%s:
              ...
              - command: test
          - deploy-to-bitrise-io@%s: {}
    default-react-native-expo-config-publish: |
      format_version: "%s"
      default_step_lib_source: https://github.com/bitrise-io/bitrise-steplib.git
      project_type: react-native
      trigger_map:
